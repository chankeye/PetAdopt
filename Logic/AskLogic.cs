...
namespace PetAdopt.Logic
{
    ...
    public class AskLogic : _BaseLogic
    {
        ...
        public IsSuccessResult DeleteAsk(int id, int userId)
        {
            ...
            if (user.IsAdmin == false && ask.OperationInfo.UserId != userId)
            {
                result.IsSuccess = false;
                result.ErrorMessage = "沒有權限";
                return result;
        ...
        {
            var log = GetLogger();
            log.Debug("title: {0}, message: {1}, classId: {2}, id: {3}, userId: {4}", data.Title, data.Message, data.ClassId, id, userId);
